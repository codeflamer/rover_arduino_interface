  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Reading left and right wheel");

  cfg_.base_link_to_front_left_wheel = info_.hardware_parameters["base_link_to_front_left_wheel"];
  cfg_.base_link_to_front_right_wheel = info_.hardware_parameters["base_link_to_front_right_wheel"];

  cfg_.base_link_to_bottom_left_wheel = info_.hardware_parameters["base_link_to_bottom_left_wheel"];
  cfg_.base_link_bottom_right_wheel = info_.hardware_parameters["base_link_bottom_right_wheel"];

  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Reading loop rate");

  cfg_.loop_rate = std::stof(info_.hardware_parameters["loop_rate"]);
  cfg_.device = info_.hardware_parameters["device"];
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Reading Baud rate");
  cfg_.baud_rate = std::stoi(info_.hardware_parameters["baud_rate"]);
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Reading timeout");
  cfg_.timeout_ms = std::stoi(info_.hardware_parameters["timeout_ms"]);
  cfg_.enc_counts_per_rev = std::stoi(info_.hardware_parameters["enc_counts_per_rev"]);
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Finished Reading");
  ...
  wheel_l_.setup(cfg_.base_link_to_front_left_wheel, cfg_.enc_counts_per_rev);
  wheel_r_.setup(cfg_.base_link_to_front_right_wheel, cfg_.enc_counts_per_rev);

  wheel_bl_.setup(cfg_.base_link_to_bottom_left_wheel, cfg_.enc_counts_per_rev);
  wheel_br_.setup(cfg_.base_link_bottom_right_wheel, cfg_.enc_counts_per_rev);
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Start Reading Encoder Value.....");
  ...
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Position Velocity Wheel..... %f %f",wheel_l_.pos,wheel_r_.vel);

  pos_prev = wheel_r_.pos;
  wheel_r_.pos = wheel_r_.calc_enc_angle();
  wheel_r_.vel = (wheel_r_.pos - pos_prev) / delta_seconds;

  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "End Read Encoder Value.....");
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Start Writing Encoder Value.....");
  // wheel_r_.cmd = 0 error, value is meat to change

  int motor_l_counts_per_loop = wheel_l_.cmd / wheel_l_.rads_per_count / cfg_.loop_rate;
  int motor_r_counts_per_loop = wheel_r_.cmd / wheel_r_.rads_per_count / cfg_.loop_rate;
  comms_.set_motor_values(motor_l_counts_per_loop, motor_r_counts_per_loop);

  RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Start Writing Encoder Value..... %f",wheel_l_.cmd);
  return hardware_interface::return_type::OK;
}
